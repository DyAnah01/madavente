...
class ArticlesController extends AbstractController
{
    ...
    public function addArticles(Request $request, EntityManagerInterface $manager, SluggerInterface $slugger)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if (!empty($form->get('images')->getData())) { //for update
                ...
                if ($fichierImage) {
                    ...
                    // On doit penser à supprimer l'ancien fichier
                    // $ancienNom =  $fichier ->gethostname
                    // unlink($dossier . $ancienNom)
                    // $originalFilename = $fichierImage->getClientOriginalName();
    ...
    public function update_articles($id, ArticlesRepository $repoA, Request $request, EntityManagerInterface $manager , SluggerInterface $slugger)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if (!empty($form->get('images')->getData())) { //for update
                ...
                if ($fichierImage) {
                    ...
                    // $originalFilename = $fichierImage->getClientOriginalName();
                    // unlink($dossier . $originalFilename);


                    $articles->setPhoto($nom);
                }
            }
            ...
            $manager->flush();
            $this->addFlash('success', "L'article N° " . $articles->getId() . " a bien été ajouté");
            return $this->redirectToRoute('add_articles');
        }
